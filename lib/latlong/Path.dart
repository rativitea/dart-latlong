...
/// Necessary for creating new instances T extends google_maps.LatLng (Path<T extends google_maps.LatLng>)
///
///     class Location extends google_maps.LatLng {
///         ....
///     }
///
///     final Path<Location> path = new Path<Location>(factory: locationFactory);
///
typedef google_maps.LatLng LatLngFactory(
    final double latitude, final double longitude);

google_maps.LatLng _defaultLatLngFactory(
        final double latitude, final double longitude) =>
    new google_maps.LatLng(latitude, longitude);

/// Path of [google_maps.LatLng] values
///
...
///     final Path<Location> path = new Path<Location>(factory: locationFactory);
///
class Path<T extends google_maps.LatLng> {
    ...
    for (int index = 0; index < coordinates.length - 1; index++) {
      ...
      if (remainingSteps <= distance ||
        ...
        for (int stepCounter = 0; stepCounter < fullSteps; stepCounter++) {
          ...
          final google_maps.LatLng tempStep =
              _distance.offset(baseStep, firstStepPos, bearing);
          final google_maps.LatLng nextStep =
  ...
  google_maps.LatLng get center {
    ...
    coordinates.forEach((final T coordinate) {
      lat = latitudeInRad(coordinate);
      lon = longitudeInRad(coordinate);
  ...
  /// Returns the [google_maps.LatLng] coordinate form [index]
  ///
  ///     final Path path = new Path.from(<google_maps.LatLng>[ startPos,endPos ]);
  ///     final google_maps.LatLng p1 = path[0]; // p1 == startPos
  ///
  ...
  CatmullRomSpline2D<double> _createSpline(
      final google_maps.LatLng p0,
      final google_maps.LatLng p1,
      final google_maps.LatLng p2,
      final google_maps.LatLng p3) {
    Validate.notNull(p0);
    Validate.notNull(p1);
    Validate.notNull(p2);
    Validate.notNull(p3);
  ...
  /// Convert [Point2D] to [google_maps.LatLng]
  google_maps.LatLng _pointToLatLng(final Point2D point) =>
